...
$query = "SELECT p.idProgramme , p.date_projection , p.heure_debut , p.num_salle , p.ticketsDisponible , p.prix_unitaire , f.titre , p.idFilm FROM programme as p , film as f WHERE f.idFilm = p.idFilm AND idProgramme=$id " ;
...
$idFilm = $programme['idFilm'];
$nomFilm = $programme['titre'];
$date_projection = $programme['date_projection'];
$heure_debut = $programme['heure_debut'];
$num_salle = $programme['num_salle'];
$ticketsDisponible = $programme['ticketsDisponible'];
$prix_unitaire = $programme['prix_unitaire'];

// Fermeture de la connexion
mysqli_close($conn);
?>

<form method="post" action="update_programme.php">
  ...
  <select name="idFilm">
    ...
    foreach ($resultat as $value) {
      ?>
      <option value="<?= $value['idFilm'] ?>" <?php if($value['idFilm']==$idFilm) {echo "selected";} ?>><?= $value['titre'] ?></option>";
    <?php
    }
    ?>
  </select>
  ...
  <input type="date" name="date_projection" value="<?php echo $date_projection; ?>"><br>
  <label for="heure_debut">heure_debut</label>
  <input type="time" name="heure_debut" value="<?php echo $heure_debut; ?>"><br>
  <label for="num_salle">num_salle</label>
  <input type="number" name="num_salle" value="<?php echo $num_salle; ?>"><br>
  <label for="ticketsDisponible">ticket Disponibles</label>
  <input type="ticketDisponible" name="ticketsDisponible" value="<?php echo $ticketsDisponible; ?>"><br>
  <label for="prix_unitaire">prix_unitaire</label>
  <input type="prix_unitaire" name="prix_unitaire" value="<?php echo $prix_unitaire; ?>"><br>
