     ...
     if(!empty($_GET['id_produit']) && isset($_SESSION['user_id']) && empty($_SESSION['panier'])){
        ...
        try{
             $pdo->beginTransaction();
               ...
               WHERE prod.id_programme = prog.idProgramme AND prod.id_user=u.idUser AND u.idUser = $_SESSION['user_id'] AND f.idGenre=g.idGenre AND prog.idFilm = f.idFilm AND prod.id_produit=$idproduit";
                $stmt=$pdo->query($req);
                ...
                if(!$isAlreadyPrinted){
                  $is_active = true ; 
                   $req2="UPDATE produit SET PDFprinted = :isactive WHERE id_produit = :productID";
                  ...
                  if($query->rowCount() > 0){
                        $pdo->commit();
                              echo (json_encode(['downloaded'=>true,"connected"=>true,"response"=>"vous avez imprimé le reçu avec succés"]));
                     $options = new Options();
