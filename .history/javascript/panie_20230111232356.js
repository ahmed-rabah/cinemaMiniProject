...
async function deleteproductFromCart(product){
    ...
    let cart = document.querySelector('.absoluteCart');
    cart.insertAdjacentElement("afterbegin",msg);
}
...
async function getUserPanier(){
    ...
    if(response === false){
        ...
        response.forEach(produit => {
            msg+=`<div class="card border-primary mb-2" style="max-width: 20rem;">
            ...
            <div class="d-flex justify-content-center">
                                <img src="${produit[0].photo}" width="60px" height="66px">
                                    ...
                                    <h6 class="btn plus" data-idprogramme="${produit.idProgramme}"><i class="uil uil-minus-circle fs-5 text-danger"></i></h6>
                                        <h6> ${produit.nbrPlace} </h6>
                                        <h6 class="btn minus" data-idprogramme="${produit.idProgramme}"><i class="uil uil-plus-circle fs-5 text-success"></i></h6>
                                        </div>
                                        <h6>Total : ${produit.prix} DH</h6>
                                        <button class="btn btn-danger" data-idprogramme="${produit.idProgramme}" onclick=deleteproductFromCart(${produit.idProgramme})><i class="uil uil-trash-alt"></i></button>
                                        </div>
                                        </div>`;
        });
    }
