...
panierBtn.addEventListener('click', () => {
    getUserPanier().then(data=>{
        console.log(data);
        cartDiv = `<div class="absoluteCart">${data}</div>`;
        document.insertAdjacentHTML("beforeend",cartDiv);   
...
async function getUserPanier(){
    ...
    if(response === false){
        ...
        response.forEach(produit => {
            msg+=`<div class="card border-primary mb-3" style="max-width: 20rem;">
            <div class="card-header">${produit[0].titre}</div>
            <div class="card-body">
                <div class="d-flex">
                <img src="${produit[0].photo}" width="70px" height="70px">
                    <div>
                        <p class="card-title">le ${produit[0].date_projection} à ${produit[0].heure_debut}</p>
                        <p class="card-text">prix_unitaire : ${produit[0].prix_unitaire}</p>
                        <p class="card-text">tickets disponible : ${produit[0].ticketsDisponible}</p>
                    </div>
                </div>
                <div class="card-footer d-flex justify-content-between">
                        <div class="d-flex">
                        <span class="plus" data-idprogramme="${produit.idProgramme}"><i class="uil uil-minus-circle"></i></span>
                        ${produit.nbrPlace}
                        <span class="minus" data-idprogramme="${produit.idProgramme}"><i class="uil uil-plus-circle"></i></span>
                        </div>
                        <h6>Total : ${produit.prix} DH</h6>
                        <button class="btn btn-danger" data-idprogramme="${produit.idProgramme}"><i class="uil uil-trash-alt bg-danger"></i>supprimer</button>
                </div>
            </div>`;
        });
    }
