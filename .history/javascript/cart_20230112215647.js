...
async function deleteproductFromCart(product){
    ...
    if(!deleted){
      ...
      cartItem.insertAdjacentHTML('beforebegin',msg);
        ...
        checkCart().then(isEmptyCart=>{if(isEmptyCart===false){
            let Container = document.getElementById('container');
            Container.innerHTML='<div class="alert alert-primary m-2 text-center" role="alert"><h2>le panier est vide</h2></div>';
        }else{
        msg=`<div class="alert alert-success alert-dismissible fade show" role="alert" id="myAlert">
        ${reponse}
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
...
function cartItemInfos(produit,use){
    ...
    if(use==="sidebar"){
        ...
        <div class="ms-1">
        <h5 class="card-title  m-0 fw-normal">le <strong>${produit[0].date_projection}</strong> à <strong>${produit[0].heure_debut}</strong></h5>
        <h5 class="card-text m-0 fw-normal">prix_unitaire :<strong> ${produit[0].prix_unitaire} DH</strong></h5>
        <h5 class="card-text m-0 fw-normal">tickets disponible :<strong> ${produit[0].ticketsDisponible}</strong> ticket(s)</h5>
        </div>
        </div>
        </div>        
        <div class="card-footer d-flex justify-content-between align-items-center p-1">
        <div class="d-flex justify-content-center align-items-center">
        <h4 class="btn minus" onclick="updateproductCart(${produit.idProgramme},'minus')"><i class="uil uil-minus-circle fs-5 text-danger"></i></h4>
        <h4> ${produit.nbrPlace} </h4>
        <h4 class="btn plus" onclick="updateproductCart(${produit.idProgramme},'plus')"><i class="uil uil-plus-circle fs-5 text-success"></i></h4>
        </div>
        <h4>Total : ${produit.prix} DH</h4>
        <button class="btn btn-danger"  onclick="deleteproductFromCart(${produit.idProgramme})"><i class="uil uil-trash-alt"></i></button>
        </div>
        </div>`;
    }
