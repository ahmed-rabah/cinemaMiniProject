...
function generatePDF(produit) {
    generator(produit).then(response=>{
        let downloaded = response.downloaded;
        ...
        if(!downloaded){
          ...
          window.location.href="./PDFgenerateur/PDFgenerator.php?id_produit="+produit;
        }
...
async function generator(produit){
    let request = await fetch("./PDFgenerateur/PDFChecker.php?id_produit="+produit);
    return request.json();
}
...
form.addEventListener('submit',(e)=>{
    ...
    changeCredentials(username,password,newpass,confirmpass).then(response=>{
        let connected = response.connected;
        ...
        parameter.insertAdjacentHTML('afterbegin',msg);
    })
})

async function changeCredentials(username,password,newpass,confirmPassword){
    let data = new URLSearchParams();
        data.append('username', username);
        data.append('password', password);
        data.append('newpassword',newpass);
        data.append('confirmerpassword',confirmPassword);
    let request = await fetch("./functions/updateParams.php", {
        method: 'POST',
        body: data,
