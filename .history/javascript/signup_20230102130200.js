    ...
    inputs.forEach(function(input){
            ...
            switch (id) {
                ...
                case 'password':
                    ...
                    break;     
                default:
                    console.edrror("something went wrong");
                    break;
...
async function existantUsernameAndEmail(username,email){
    let response = await fetch('./functions/emailChecking.php?username='+username+'&email='+email);
                let data= await response.json();
                // .then(data => {result= data;}).catch(err => {console.log(err)});
     return data;
}
async function usernamevalidation(value){
    let response="";
    let state=false;
    let data =await  existantUsername(value);
        if(value === ''){
            ...
            response ="username existe déja";
        }else if(data == false && valideUsername(value)){
