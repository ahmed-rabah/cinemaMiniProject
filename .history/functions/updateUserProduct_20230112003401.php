...
if($_SERVER['REQUEST_METHOD'] == 'GET'){
    ...
    if(!empty($_GET['status']) && !empty($_GET['idProgramme']) && isset($_SESSION['user_id'])){
        ...
        if($checkreq->rowCount() <= 0){
            exit(json_encode(['updated'=>false,'connected'=>true,'response'=>"programme introuvable"]));
        }
        $status = $_GET['status'];
        ...
        if($exist){
            ...
            if($status=="minus"){
                ...
                $_SESSION['panier'] = $panier;
                exit(json_encode(['updated'=>false,'connected'=>true,'response'=>'vous devez acheter au moins une ticket']));
                }
            }
        ...
        if(!$exist){
        exit(json_encode(['updated'=>false,'connected'=>true,'response'=>'le programme que vous voulez modifier est introuvable dans le panier']));
        }

    }else{
        exit(json_encode(['updated'=>false,'connected'=>false,'response'=>"pour ajouter au panier vous devez être connécté "]));
    }
}